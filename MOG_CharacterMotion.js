...
Game_Interpreter.prototype.checkFollowerCharEffects = function (command, args) {
	...
	if (command === "breath_mode_follower_id") {
		var bparty_id = Number(args[1]);
		npk = bparty_id === 0 ? 0 : 1
	};
	if (command === "ghost_mode_follower_id") {
		var gparty_id = Number(args[1]);
		npk = gparty_id === 0 ? 0 : 1
	};
	if (command === "float_mode_follower_id") {
		var fparty_id = Number(args[1]);
		npk = fparty_id === 0 ? 0 : 1
	};
	if (command === "swing_mode_follower_id") {
		var sparty_id = Number(args[1]);
		npk = sparty_id === 0 ? 0 : 1
	};
	if (command === "rotation_follower_id") {
		var rparty_id = Number(args[1]);
		npk = rparty_id === 0 ? 0 : 1
	};
	if (command === "zoom_follower_id") {
		var zparty_id = Number(args[1]);
		npk = zparty_id === 0 ? 0 : 1
	};
	if (command === "collapse_effect_follower_id") {
		var colparty_id = Number(args[1]);
		npk = colparty_id === 0 ? 0 : 1
	};
	if (command === "clear_effects_follower_id") {
		var cparty_id = Number(args[1]);
		npk = cparty_id === 0 ? 0 : 1
	};
	if (command === "shake_effects_follower_id") {
		var shparty_id = Number(args[1]);
...
Game_Interpreter.prototype.checkEventsCharEffects = function (command, args) {
	...
	if (command === "collapse_effect_event_id") { var colevent_id = Number(args[1]); nck = true };
