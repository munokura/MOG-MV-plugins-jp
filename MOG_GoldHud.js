...
/*:ja
 * @plugindesc (v1.6f) マップ画面で所持金をHUD表示します。
 * @author Moghunter
 *
 * @param Initial Visible
 * @text 開始時のHUD表示
 ...
 * (v1.6) - コーディングとプラグインの互換性の向上
 * (v1.5) - Chrono Engineとの互換性
 * (v1.4) - 点滅するHUDのグリッチ修正
 * (v1.3) - ゲーム開始時にhide hudを追加
 * (v1.2) - 非表示モードでのHUDの点滅修正
 * (v1.1) - セットアップによるHUD位置の修正
 *
 */
...
Moghunter.ghud_fade_limit = Number(Moghunter.parameters['Fade Limit'] || 60);
...
Gold_Hud.prototype.update_number = function () {
	...
	if (this._number_old != dif_number) {
		this._number_old = dif_number;
		this.refresh_number(this._number, this._number_old, this._number_img_data, this._number_img_data[4], 0);
	};
};
